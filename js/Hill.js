function encrypt(plaintext, k) { 
  ...
  if(keys.length != 4){ alert("Түлхүүр нь 4 тооноос бүрдсэн байх ёстой"); return; }
...
function decrypt(ciphertext, k) {
  keys = k.split(" "); 
  if(ciphertext.length % 2 == 1){ alert("Шифрлэгдсэн мессеж нь 2-д хуваагдахгүй байна (алгоритм буруу байна уу?)"); return; }
  if(keys.length != 4){ alert("Түлхүүр нь 4 тооноос бүрдсэн байх ёстой"); return; }
  for(i=0;i<4;i++) keys[i] = keys[i]%26;
  ...
  if(di == 0){alert("Түлхүүрийг эргүүлэх боломжгүй, өөр түлхүүр ашиглана уу"); return; }
  ikeys = new Array(4);
